...
const ManagerBan = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
              // @ts-ignore
              const response  = await getAllManagers(token, page, rowsPerPage);
              if (response.ok) {
    ...
    const columns = [
        ...
        { field: "sportsHall", headerName: "Sports hall", flex: 1 },
        { field: "dateEmployment", headerName: "Date Employment", flex: 1 },
        {
            field: "role",
            headerName: "Role",
            flex: 1,
    ...
    async function handleBanUser(id: any): Promise<void> {
        const response  = await banManager(token||"", id);
        if (response.ok) {
          const res  = await getAllManagers(token||"", page, rowsPerPage);
          if (res.ok) {
    ...
    async function handleUnbanUser(id: any): Promise<void> {
        const response  = await unbanManager(token||"", id);
        if (response.ok) {
          const res  = await getAllManagers(token||"", page, rowsPerPage);
              if (res.ok) {
                const ans = (await res.json());
